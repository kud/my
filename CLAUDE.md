...
- **Be surgical.** Touch the _fewest files and lines_ needed for the task.
...
- All core scripts are **Zsh**. Do _not_ convert files to bash.
...
- `core/main.zsh` – master environment orchestrator (setup\_\* functions). Add new cross‑cutting setup stages here.
...
Place new logic where its _primary responsibility_ fits best, rather than where it is first needed.
...
## AI Assets (Agents, Skills, Commands)

Never create or edit agents/skills/commands directly in `~/.claude/`. That directory is managed by `my ai sync` and only contains symlinks. Always work in the source directories:

- **Global** (not profile-specific): `$MY/ai/agents/`, `$MY/ai/skills/`, `$MY/ai/commands/`
- **Profile-specific** (prefixed with the profile name): `$MY/profiles/$PROFILE/ai/agents/`, `$MY/profiles/$PROFILE/ai/skills/`, `$MY/profiles/$PROFILE/ai/commands/`

After creating or modifying agents/skills, run `my ai sync` to update the symlinks.

---

## AI Config (Settings & MCP Servers)

Never edit `~/.claude/settings.json` directly. All Claude Code configuration lives in the managed config files:

- **Global** (all profiles): `$MY/config/cli/claude-code.yml`
- **Profile-specific**: `$MY/profiles/$PROFILE/config/cli/claude-code.yml`

This includes MCP servers (`mcp:` block), permissions, hooks, and any other settings. Changes here are applied by the `my` config sync system — `~/.claude/` is a derived output, not a source of truth.
